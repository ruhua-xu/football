...
from football_system.domain.match import MarketOddsSnapshot, OddsQuote, ProviderMatchMapping
    ...
    async def fetch_market_odds(self, query: SnapshotQuery) -> MarketOddsBatch:
        ...
        mappings = tuple(
            ProviderMatchMapping(
                mapping_id=stable_id("mapping", self.provider_code, seed.market_external_id),
...
def _snapshot(seed: MockMatchSeed, market: MarketKey) -> MarketOddsSnapshot:
    ...
    return MarketOddsSnapshot(
        snapshot_id=stable_id("market-odds", MockMarketOddsProvider.provider_code, version_key),
